...
#include "./VulkanExp/PrimCommandBuffers.h"
#include "./VulkanExp/PrimRenderPass.h"
...
private:
	...
	PrimRenderPass* renderPass;

	GraphicsPipeline* graphicsPipeline;

	CommandPool* commandPool;

	PrimCommandBuffers* commandBuffers;
	...
	*/
	...
	void initVulkan() {
		instance = new Instance("Hello Triangle", "No Engine", true);
		...
		renderPass = new PrimRenderPass(*device, *swapChain);
		graphicsPipeline = new GraphicsPipeline(*device, *swapChain, *renderPass);
		commandPool = new CommandPool(*device, 0);
		commandBuffers = new PrimCommandBuffers(*device, *renderPass, *swapChain, *graphicsPipeline, *commandPool);
	...
	QueueFamilyIndices findQueueFamilies(VkPhysicalDevice device) {
		...
		//Iterate through queueFamilies to find one that supports VK_QUEUE_GRAPHICS_BIT
		int i = 0;
	...
	VkExtent2D chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
		if (capabilities.currentExtent.width != std::numeric_limits<uint32_t>::max()) {
			...
			int width, height;
			//TODO: we shouldn't get access from window.hpp
	...
	void createGraphicsPipeline() {
		...
		//referance all structs describing the fixed-function stage
		...
		//FINALLY CREATING GRAPHICS PIPELINE
	...
	void createFramebuffers() {
		...
		//iterate through image views and create framebuffers from each
	...
	void drawFrame(bool& framebufferResized) {
		...
		uint32_t imageIndex;
		VkResult result = vkAcquireNextImageKHR(device->logical(), swapChain->handle(), UINT64_MAX, imageAvailableSemaphores[currentFrame], VK_NULL_HANDLE, &imageIndex);
		...
		//keep resetFence call after we know we are submitting work
	...
	void createTextureSampler() {
		...
		//tutorial has this as false but it causes a lot of validation errors due to the combination of this
		// the magFilter and minFilter, and the VK_FORMAT_R8G8B8A8_SRGB image
